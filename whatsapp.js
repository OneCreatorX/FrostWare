...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const sessionId = req.headers["session-id"] || req.query.session,
      sessionDir = join(__dirname, "uploads", sessionId || "temp")
...
const mediaDir = join(__dirname, "media"),
  ...
  cookiesDir = join(__dirname, "cookies")
try {
...
app.post("/api/cookies", upload.single("cookiesFile"), async (req, res) => {
  try {
    ...
    // Also save original JSON for reference
...
app.post("/api/formats", async (req, res) => {
  try {
    ...
    const userAgent = getRandomUserAgent(),
      headers = createBrowserHeaders(userAgent)
...
app.post("/api/download-format", async (req, res) => {
  try {
    const { url, formatId } = req.body,
      sessionId = req.headers["session-id"] || req.query.session
    ...
    const sock = activeSessions.get(sessionId),
      ...
      tempFilePath = join(tempDir, `${Date.now()}_${crypto.randomBytes(8).toString("hex")}`)
    ...
    const downloadedFile = join(tempDir, files[0]),
      ...
      fileBuffer = fs.readFileSync(downloadedFile)
    await sock.sendMessage(sock.user.id, { document: fileBuffer, fileName: files[0], mimetype: getMimeType(files[0]) })
...
app.post("/api/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    await sock.sendMessage(sock.user.id, { document: fileBuffer, fileName: file.originalname, mimetype: file.mimetype })
...
app.get("/api/status", async (req, res) => {
  try {
    ...
    if (!activeSessions.has(sessionId)) createWhatsAppSession(sessionId)
...
async function createWhatsAppSession(sessionId) {
  ...
  try {
      ...
      if (qr) {
        try {
          const qrImage = await qrcode.toDataURL(qr, { width: 300, margin: 2 }),
            base64Data = qrImage.replace(/^data:image\/png;base64,/, ""),
              ...
              if (CONFIG.DOWNLOAD_MEDIA_BY_DEFAULT) {
                ...
                if (media) {
                  const mediaType = getMediaType(message),
                    filename = getMediaFilename(message, mediaType),
                  ...
                  await saveMessage(sessionId, {
                    ...
                    text: "",
                    timestamp: new Date(message.messageTimestamp * 1000).toISOString(),
                    media: { type: mediaType, filename: filename, size: formatFileSize(media.length) },
                  })
    ...
    if (sock) sock.ws.on("CB:call", (json) => {})
  }
}
...
function getMediaFilename(message, mediaType) {
  ...
  const extensions = { image: "jpg", video: "mp4", audio: "mp3", document: "bin" }
...
async function saveMessage(sessionId, messageData) {
  try {
    const sessionDir = join(__dirname, "sessions", sessionId),
      messagesFile = join(sessionDir, "messages.json")
    let messages = []
  ...
  try {
    ...
    tempFiles.forEach((file) => {
      const filePath = join(tempDir, file),
        ...
        ageInMinutes = (now - stats.mtime.getTime()) / (1000 * 60)
      if (ageInMinutes > 30) fs.unlinkSync(filePath)
    })
  } catch (error) {}
}, 600000)
  ...
  try {
    ...
    mediaFiles.forEach((file) => {
      const filePath = join(mediaDir, file),
        stats = fs.statSync(filePath),
        ageInHours = (now - stats.mtime.getTime()) / (1000 * 60 * 60)
      if (ageInHours > 24) fs.unlinkSync(filePath)
    })
  } catch (error) {}
}, 3600000)
