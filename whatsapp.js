...
function convertJsonToNetscape(cookies) {
  ...
  cookies.forEach((cookie) => {
    ...
    // Fix for expiration date format - ensure it's an integer
    let expiration = cookie.expirationDate || cookie.expires || Math.floor(Date.now() / 1000) + 365 * 24 * 60 * 60
    expiration = Math.floor(expiration) // Remove decimal part
  ...
  try {
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      if (connection === "close") {
        ...
        if (fs.existsSync(join(sessionDir, "qr.png"))) fs.unlinkSync(join(sessionDir, "qr.png"))
    ...
    sock.ev.on("messages.upsert", async (m) => {
      ...
      for (const msg of m.messages) {
        ...
        if (msg.message?.documentMessage) {
          ...
          const mediaKey = `${document.directPath}_${document.mediaKeyTimestamp}`

          if (!mediaCache.has(mediaKey) && CONFIG.DOWNLOAD_MEDIA_BY_DEFAULT) {
            try {
              const buffer = await downloadMediaMessage(msg, "buffer", {})
              const filename = `${Date.now()}_${document.fileName || "file"}`
              const filePath = join(mediaDir, filename)
              fs.writeFileSync(filePath, buffer)

              mediaCache.set(mediaKey, {
                filename,
                type: "document",
                mimetype: document.mimetype,
                size: formatBytes(document.fileLength),
              })
            } catch (error) {}
          }

          if (mediaCache.has(mediaKey)) {
            messageData.media = mediaCache.get(mediaKey)
          }
        } else if (msg.message?.imageMessage) {
          const image = msg.message.imageMessage
          const mediaKey = `${image.directPath}_${image.mediaKeyTimestamp}`

          if (!mediaCache.has(mediaKey) && CONFIG.DOWNLOAD_MEDIA_BY_DEFAULT) {
            try {
              const buffer = await downloadMediaMessage(msg, "buffer", {})
              const filename = `${Date.now()}_image.${image.mimetype.split("/")[1]}`
              const filePath = join(mediaDir, filename)
              fs.writeFileSync(filePath, buffer)
              ...
              mediaCache.set(mediaKey, {
                ...
                size: formatBytes(image.fileLength),
              })
            } catch (error) {}
          }
          ...
          const mediaKey = `${video.directPath}_${video.mediaKeyTimestamp}`

          if (!mediaCache.has(mediaKey) && CONFIG.DOWNLOAD_MEDIA_BY_DEFAULT) {
            try {
              const buffer = await downloadMediaMessage(msg, "buffer", {})
              const filename = `${Date.now()}_video.${video.mimetype.split("/")[1]}`
              const filePath = join(mediaDir, filename)
              fs.writeFileSync(filePath, buffer)
              ...
              mediaCache.set(mediaKey, {
                ...
                size: formatBytes(video.fileLength),
              })
            } catch (error) {}
          }
          ...
          const mediaKey = `${audio.directPath}_${audio.mediaKeyTimestamp}`

          if (!mediaCache.has(mediaKey) && CONFIG.DOWNLOAD_MEDIA_BY_DEFAULT) {
            try {
              const buffer = await downloadMediaMessage(msg, "buffer", {})
              const filename = `${Date.now()}_audio.${audio.mimetype.split("/")[1]}`
              const filePath = join(mediaDir, filename)
              fs.writeFileSync(filePath, buffer)
              ...
              mediaCache.set(mediaKey, {
                ...
                size: formatBytes(audio.fileLength),
              })
            } catch (error) {}
          }
