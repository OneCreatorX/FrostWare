...
local downloadProgressFrame = nil
local downloadProgressBar = nil
local downloadProgressLabel = nil
...
local function showDownloadProgress(filename, progress, downloadedSize, totalSize)
    if not downloadProgressFrame then
        downloadProgressFrame = FW.cF(mp, {
            BackgroundColor3 = Color3.fromRGB(25, 30, 40),
        ...
        local progressFrame = FW.cF(downloadProgressFrame, {
            BackgroundColor3 = Color3.fromRGB(30, 35, 45),
    ...
    if downloadProgressBar and downloadProgressLabel then
        downloadProgressBar.Size = UDim2.new(progress / 100, 0, 1, 0)
        if downloadedSize and totalSize then
            downloadProgressLabel.Text = math.floor(progress) .. "% - " .. fs(downloadedSize) .. "/" .. fs(totalSize)
        else
            downloadProgressLabel.Text = math.floor(progress) .. "%"
        end
    end
end

local function hideDownloadProgress()
    if downloadProgressFrame then
        downloadProgressFrame:Destroy()
        downloadProgressFrame = nil
...
local function scanLocalMusicFiles(directory)
    ...
    for _, filePath in ipairs(result) do
        if isfile(filePath) then
            ...
            if filename then
                ...
                if ext then
                    ext = "." .. ext:lower()
                    for _, supportedExt in ipairs(sf) do
        ...
        elseif isfolder(filePath) then
            local subFiles = scanLocalMusicFiles(filePath)
...
local function dlCloudFile(song, callback)
    ...
    spawn(function()
        ...
        showDownloadProgress(song.filename, 0)
        ...
        local chunkSize = 1024 * 100
        local downloadedSize = 0
        local position = 0
        
        while position < #data do
            local endPos = math.min(position + chunkSize, #data)
            local chunk = string.sub(data, position + 1, endPos)
            downloadedSize = downloadedSize + #chunk
            
            local progress = (downloadedSize / totalSize) * 100
            downloadProgress[song.filename] = progress
            showDownloadProgress(song.filename, progress, downloadedSize, totalSize)
            if callback then callback("downloading", progress) end
        ...
        showDownloadProgress(song.filename, 100, totalSize, totalSize)
    ...
    if scrollFrame then
        ...
        for i = 1, #sortedMusic do
            ...
            if downloadProgress[dt.filename] then
                ...
                cUI(mc, "text", {
                    Text = "Downloading... " .. math.floor(prog) .. "%",
                    TextSize = 12,
