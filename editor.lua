...
local function mkimgbtn(parent,name,asset,sz,ps,zi,fn,bg,circle)local b=Instance.new("ImageButton")b.Parent=parent;b.Name=name;b.BackgroundColor3=bg or Color3.fromRGB(120,95,205);b.BorderSizePixel=0;b.BackgroundTransparency=1;b.Size=sz or UDim2.new(0,120,0,36);b.Position=ps or UDim2.new(0,0,0,0);b.ZIndex=zi or 1;b.Image=asset;b.ScaleType=Enum.ScaleType.Fit;if circle then local uc=Instance.new("UICorner")uc.Parent=b;uc.CornerRadius=UDim.new(1,0)end;if fn then if y then y(b,fn)else b.MouseButton1Click:Connect(fn)end end;return b end
...
if not bx then bx=Instance.new("TextBox")bx.Parent=ed;bx.Name="EditorBox";bx.BorderSizePixel=0;bx.BackgroundColor3=Color3.new(0.8,0.8,0.8);bx.BackgroundTransparency=0;bx.Size=UDim2.new(contentXScale,0,editorH,0);bx.Position=UDim2.new(MARGIN,0,editorY,0);bx.TextColor3=W;bx.TextSize=15;bx.TextWrapped=true;bx.MultiLine=true;bx.ClearTextOnFocus=false;bx.Font=Enum.Font.SourceSans;local uc=Instance.new("UICorner")uc.Parent=bx;uc.CornerRadius=UDim.new(0.06,0)else bx.Size=UDim2.new(contentXScale,0,editorH,0)bx.Position=UDim2.new(MARGIN,0,editorY,0)end
...
local function restyle()for n,t in pairs(tabs)do local a=(n==cur)t.h.BackgroundColor3=a and PRIMARY_ACTIVE or PRIMARY;if t.sel then t.sel.Visible=a end end end
...
local ord=1
local function mkTab(name)local px=math.max(62,(#name*8)+(name~="Main"and 34 or 22)+20)local hold=Instance.new("Frame")hold.Parent=hdr;hold.Name="T-"..name;hold.BorderSizePixel=0;hold.BackgroundColor3=PRIMARY;hold.BackgroundTransparency=0;hold.Size=UDim2.new(0,px,1,0);hold.ZIndex=2;hold.LayoutOrder=ord;ord=ord+1;local uc=Instance.new("UICorner")uc.Parent=hold;uc.CornerRadius=UDim.new(0.8,0);local us=Instance.new("UIStroke")us.Parent=hold;us.ApplyStrokeMode=Enum.ApplyStrokeMode.Border;us.Thickness=1;local lab=Instance.new("TextLabel")lab.Parent=hold;lab.Name="L";lab.Text=name;lab.TextSize=13;lab.TextColor3=BTN_TEXT;lab.Font=Enum.Font.SourceSans;lab.BorderSizePixel=0;lab.BackgroundTransparency=1;lab.Size=UDim2.new(1,0,1,0);lab.Position=UDim2.new(0,0,0,0);lab.TextXAlignment=Enum.TextXAlignment.Center;lab.TextYAlignment=Enum.TextYAlignment.Center;local hit=Instance.new("TextButton")hit.Parent=hold;hit.Name="Hit";hit.BorderSizePixel=0;hit.BackgroundTransparency=1;hit.Size=UDim2.new(1,0,1,0);hit.Position=UDim2.new(0,0,0,0);hit.ZIndex=3;hit.Text="";hit.MouseButton1Click:Connect(function()selectTab(name)end);local sel=Instance.new("Frame")sel.Parent=hold;sel.Name="Sel";sel.BorderSizePixel=0;sel.BackgroundColor3=PRIMARY_LIGHT;sel.BackgroundTransparency=0;sel.Size=UDim2.new(1,0,0,3);sel.Position=UDim2.new(0,0,1,-3);sel.ZIndex=4;sel.Visible=false;local uc2=Instance.new("UICorner")uc2.Parent=sel;uc2.CornerRadius=UDim.new(1,0);local close=nil;if name~="Main"then close=Instance.new("TextButton")close.Parent=hold;close.Name="X";close.BorderSizePixel=0;close.BackgroundColor3=PRIMARY_ACTIVE;close.TextColor3=BTN_TEXT;close.Text="X";close.TextSize=12;close.Font=Enum.Font.SourceSans;close.Size=UDim2.new(0,16,0,16);close.Position=UDim2.new(1,-22,0,7);close.BackgroundTransparency=0;close.ZIndex=5;local uc3=Instance.new("UICorner")uc3.Parent=close;uc3.CornerRadius=UDim.new(1,0);close.MouseButton1Click:Connect(function()if tabs[name]then if cur==name then cur="Main"selectTab("Main")end;pcall(function()if isfile(fpath(name))then delfile(fpath(name))end end)tabs[name]=nil;hold:Destroy()restyle()end end)end;tabs[name]={h=hold,l=lab,x=close,sel=sel}return hold end
...
local pv=Instance.new("Frame")pv.Parent=ov2;pv.Name="EdAddPv";pv.BorderSizePixel=0;pv.BackgroundColor3=Color3.new(0.6,0.6,0.6);pv.BackgroundTransparency=0;pv.Size=UDim2.new(0,300,0,140);pv.Position=UDim2.new(0.5,-150,0.5,-70);pv.ZIndex=1000
local uc4=Instance.new("UICorner")uc4.Parent=pv;uc4.CornerRadius=UDim.new(0.24,0)
local us2=Instance.new("UIStroke")us2.Parent=pv;us2.ApplyStrokeMode=Enum.ApplyStrokeMode.Border;us2.Thickness=1
...
local uc5=Instance.new("UICorner")uc5.Parent=nb;uc5.CornerRadius=UDim.new(0.24,0)
...
local plus=mkimgbtn(hdr,"Plus",addAsset,UDim2.new(0,28,0,28),UDim2.new(0,0,0,0),2147480000,function()nb.Text=""ov2.Visible=true end,nil,true)
...
local bar=Instance.new("Frame")bar.Parent=ed;bar.Name="EdBar";bar.BorderSizePixel=0;bar.BackgroundColor3=Color3.new(1,1,1);bar.BackgroundTransparency=1;bar.Size=UDim2.new(contentXScale,0,BAR_H,0);bar.Position=UDim2.new(MARGIN,0,barY,0)
local uc6=Instance.new("UICorner")uc6.Parent=bar;uc6.CornerRadius=UDim.new(0.16,0)
local us3=Instance.new("UIStroke")us3.Parent=bar;us3.ApplyStrokeMode=Enum.ApplyStrokeMode.Border;us3.Thickness=1
...
local function showToast(text)if bar:FindFirstChild("Toast")then bar.Toast:Destroy()end;local tl=Instance.new("TextLabel")tl.Name="Toast";tl.Parent=bar;tl.BackgroundColor3=Color3.new(0,0,0);tl.BackgroundTransparency=0.25;tl.TextColor3=Color3.new(1,1,1);tl.Font=Enum.Font.SourceSansSemibold;tl.TextSize=14;tl.Text=text;tl.Size=UDim2.new(0,180,0,26);tl.Position=UDim2.new(0.5,-90,0,6);tl.AnchorPoint=Vector2.new(0,0);tl.TextXAlignment=Enum.TextXAlignment.Center;tl.TextYAlignment=Enum.TextYAlignment.Center;local uc=Instance.new("UICorner")uc.Parent=tl;uc.CornerRadius=UDim.new(0.5,0);tl.TextTransparency=1;local t1=TS:Create(tl,TweenInfo.new(0.12,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{TextTransparency=0});t1:Play();task.delay(0.85,function()if tl and tl.Parent then local t2=TS:Create(tl,TweenInfo.new(0.15,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{TextTransparency=1});t2:Play();t2.Completed:Wait();if tl then tl:Destroy()end end end)end
local function attachPressFeedback(btn,msg)local sc=Instance.new("UIScale")sc.Parent=btn;sc.Scale=1;btn.MouseButton1Down:Connect(function()TS:Create(sc,TweenInfo.new(0.08,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Scale=0.92}):Play()end)btn.MouseButton1Up:Connect(function()TS:Create(sc,TweenInfo.new(0.10,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Scale=1}):Play()end)btn.MouseLeave:Connect(function()TS:Create(sc,TweenInfo.new(0.10,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Scale=1}):Play()end)btn.MouseButton1Click:Connect(function()if msg then showToast(msg)end end)end
...
local function addBarImg(name,asset,onClick,label)local b=mkimgbtn(bar,name,asset,UDim2.new(0.22,0,0,36),UDim2.new(0,0,0,0),1,function()onClick()end,nil,false)attachPressFeedback(b,label)return b end
...
addBarImg("Exec",icExec,function()runCode(bx.Text or "")end,"Execute")
addBarImg("Clear",icClear,function()bx.Text=""end,"Clean")
addBarImg("Paste",icPaste,function()local s=(getclipboard and getclipboard())or"";if #s>0 then bx.Text=s end end,"Paste")
addBarImg("ExecClip",icExecCb,function()local s=(getclipboard and getclipboard())or"";runCode(s)end,"Execute")
attachPressFeedback(plus,"New Tab")
